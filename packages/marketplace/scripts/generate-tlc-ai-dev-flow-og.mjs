...
const sideItems = ['v1 core loop', 'Event-driven queue', 'Layered verification', 'More stations coming']
...
const CONTENT_X = 120
const BADGE_LABEL = 'AGENTIC SOFTWARE FACTORY'
...
const BADGE_TEXT_W = Math.ceil(
  BADGE_LABEL.length * BADGE_FONT_SIZE * 0.62 + BADGE_LABEL.length * BADGE_FONT_SIZE * BADGE_LETTER_SPACING + 8,
)
...
<svg width="1200" height="630" viewBox="0 0 1200 630" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="${CONTENT_X}" y="210" fill="#F8FAFC" font-family="system-ui, -apple-system, Segoe UI, sans-serif" font-size="54" font-weight="800" letter-spacing="-0.03em">TLC AI Dev Flow</text>
  <text x="${CONTENT_X}" y="258" fill="#CBD5E1" font-family="system-ui, -apple-system, Segoe UI, sans-serif" font-size="26" font-weight="500">Skills that run the factory, not the session</text>
  <rect x="${CONTENT_X}" y="278" width="420" height="3" rx="1.5" fill="url(#accent)"/>

  ${phaseFlow}
  ${phaseArrows}

  <text x="${CONTENT_X}" y="${ADAPTIVE_Y}" fill="#64748B" font-family="system-ui, -apple-system, Segoe UI, sans-serif" font-size="15" font-weight="600" letter-spacing="0.06em">v1 OF THE LINE</text>

  ${featureChecks}

  <rect x="780" y="290" width="300" height="198" rx="16" fill="#0F172A" fill-opacity="0.72" stroke="#334155" stroke-opacity="0.8"/>
  <text x="804" y="322" fill="#94A3B8" font-family="system-ui, -apple-system, Segoe UI, sans-serif" font-size="13" font-weight="700" letter-spacing="0.1em">FACTORY</text>
  ${sideList}
</svg>`

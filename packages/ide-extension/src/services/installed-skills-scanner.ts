...
export class InstalledSkillsScanner {
   ...
   *
   * @param ports - Core ports for agent path resolution.
  ...
  private getAgentConfigs(): AgentScanConfig[] {
    return getAgentPathConfigs(this.ports).map((agent) => ({
      name: agent.agent,
  ...
  private async scanSkill(
    ...
    for (const config of this.getAgentConfigs()) {
  ...
  getAllAgents(): AvailableAgent[] {
    return this.getAgentConfigs().map((config) => ({
      agent: config.name,
      displayName: config.displayName,
      company: config.company,
    }))

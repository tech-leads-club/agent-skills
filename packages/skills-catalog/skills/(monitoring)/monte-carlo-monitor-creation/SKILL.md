...
description: Create Monte Carlo monitors via MCP tools, producing monitors-as-code YAML for CI/CD deployment. Use when user says "add a monitor", "create a freshness check", "set up validation", or "generate monitors-as-code". Do NOT use for triaging alerts, running impact assessments, or editing existing monitors.
metadata:
  version: '1.0.0'

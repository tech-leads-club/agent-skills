...
description: Push metadata, lineage, and query logs to Monte Carlo from any data warehouse via the pycarlo SDK. Use when user says "push data to Monte Carlo", "build a collection script", "push metadata", "push lineage", or "why isn't my pushed data showing up". Do NOT use for pull-based integrations or Monte Carlo UI configuration.
metadata:
  version: '1.0.0'

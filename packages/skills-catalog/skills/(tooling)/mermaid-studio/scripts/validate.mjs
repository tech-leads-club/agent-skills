...
function basicValidation(content, _filename) {
  ...
  const reservedWords = ['end', 'graph', 'subgraph', 'style', 'class', 'click', 'linkStyle', 'default']
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const word of reservedWords) {
      ...
      if (pattern.test(trimmed) && !trimmed.includes(`${word}[`) && !trimmed.includes(`${word}(`)) {
        warnings.push(`Line ${i + 1}: "${word}" is a reserved keyword. Use "${word}Node" or "${word}State" instead.`)
      }
    }
  }
      ...
      if (labelMatch) {
        for (const label of labelMatch) {
          if (label.includes('"') && !label.startsWith('["')) {
            warnings.push(`Line ${i + 1}: Unquoted label with double quotes. Wrap in ["..."]: ${label.slice(0, 30)}`)
          }
        }
      }
    }

...
async function renderFile(inputFile, outputFile, format, theme, timeout) {
  const renderScript = format === 'ascii' ? resolve(__dirname, 'render-ascii.mjs') : resolve(__dirname, 'render.mjs')
...
async function runBatch(files, opts) {
  ...
  for (let i = 0; i < files.length; i += opts.workers) {
    ...
    const promises = chunk.map((inputFile) => {
      const outputFile = getOutputPath(inputFile, resolve(opts.inputDir), resolve(opts.outputDir), opts.format)

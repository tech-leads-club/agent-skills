...
function isPuppeteerAvailable() {
  const paths = [resolve(DEPS_DIR, 'node_modules/puppeteer'), resolve(DEPS_DIR, 'node_modules/puppeteer-core')]
  for (const p of paths) {
    if (existsSync(p)) return p
  }
  return null
...
async function renderWithPuppeteer(inputFile, outputFile, opts) {
  ...
  try {
    ...
    const iconRegistrations = opts.icons
      ...
      .map((name) => `{ name: '${name}', loader: () => fetch('${ICON_PACK_URLS[name]}').then(r => r.json()) }`)
      .join(',\n      ')

...
description: "Free web search via DuckDuckGo — text, news, images, videos. No API key required. Supports CLI (ddgs) and Python library modes with region filtering, time filtering, and JSON output. Use when "search the web", "find information about", "look up", "news search", or when web_search is unavailable. Do NOT use for site-specific searches (use the site directly), academic papers (use arxiv), or real-time data."
license: CC-BY-4.0
metadata:
  version: '1.0.0'

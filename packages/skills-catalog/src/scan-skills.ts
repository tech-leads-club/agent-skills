  ...
  try {
    ...
    if (toScan.length > 0) {
      ...
      try {
        ...
        execSync('uvx --refresh mcp-scan==0.1.17 --help', { stdio: 'ignore' })
...
async function scanSkill(skill: SkillInfo): Promise<ScanIssue[]> {
  return new Promise((resolve) => {
    ...
    const proc = spawn('uvx', ['mcp-scan==0.1.17', '--skills', skill.dir, '--json'], { timeout: 120_000 })
        ...
        resolve([
          {
            ...
            message: `Failed to parse snyk-agent-scan output: ${msg}`,
            reference: [0, null],
            extra_data: {
              risk_score: 10,
